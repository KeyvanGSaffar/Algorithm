...
public class Point implements Comparable<Point> {
   ...
   * - x0). For completeness, the slope is defined to be +0.0 if the line
   * segment connecting the two points is horizontal; Double.POSITIVE_INFINITY
   * if the line segment is vertical; and Double.NEGATIVE_INFINITY if (x0, y0)
   * and (x1, y1) are equal.
   *
   * @param that
   *          the other point
   ...
   * y1) if and only if either y0 < y1 or if y0 = y1 and x0 < x1.
   *
   * @param that
   *          the other point
